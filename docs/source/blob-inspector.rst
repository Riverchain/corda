...
disadvantage is the inability to view the contents in human-friendly manner. The blob inspector tool alleviates this issue
by allowing the contents of a binary blob file (or URL end-point) to be output in either YAML or JSON. It uses
``JacksonSupport`` do this (see :doc:`json`).
...
see this when dealing with classes such as ``SignedData`` or other structures that attach a signature, such as the node-info
files or the ``network-parameters`` file in the node's directory. For example, the output of a node-info file may look like:
...
This property is materialised into the ``NodeInfo`` and is output under the ``deserialized`` field.
